...
// Cỡ tàu mẫu để tính. fuelLiters = lít dầu cả chuyến (nguồn: Dân trí 27/2/2022 tàu 400 CV 10 tới 15 ngày
// 1.500 tới 2.000 lít; VnExpress 4/7/2005 tàu 500 tới 600 CV khoảng 1 tháng 10.000 lít).
export const BENEFIT_CASES = [
  { key: 'ghe-nho', label: 'ghe 10 tới 12 m, máy 90 tới 150 cv, 5 người, chuyến 5 tới 7 ngày', crew: 5, days: 6, fuelLiters: null },
  { key: 'tau-15m', label: 'tàu 15 m, máy 400 cv, 10 người, chuyến 15 ngày', crew: 10, days: 15, fuelLiters: 2000, fuelSource: 'Dân trí 27/2/2022, tàu 400 cv Thanh Hóa 1.500 tới 2.000 lít mỗi chuyến 10 tới 15 ngày' },
  { key: 'tau-20m', label: 'tàu 18 tới 20 m, máy 500 tới 600 cv, 12 người, chuyến 30 ngày', crew: 12, days: 30, fuelLiters: 10000, fuelSource: 'VnExpress 4/7/2005, tàu 500 tới 600 cv khoảng 10.000 lít mỗi chuyến một tháng' },
];
export function vnd(n) {
  return Math.round(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ' đ';
}
export function trieu(n) {
  return (Math.round(n / 100000) / 10).toString().replace('.', ',') + ' triệu đồng';
}
...
export function fuelBenefitLines() {
  ...
  for (const c of BENEFIT_CASES) {
    if (!c.fuelLiters) continue;
    const cost = c.fuelLiters * a.dieselPrice;
    const [lo, hi] = a.fuelSavingPct;
    const saveLo = cost * lo / 100; const saveHi = cost * hi / 100;
    out.push(`VÍ DỤ ${c.label}: ${c.fuelLiters.toLocaleString('vi-VN')} lít dầu một chuyến (${c.fuelSource}) × ${vnd(a.dieselPrice)}/lít (${a.dieselPriceSource}, kỳ ${a.dieselPriceDate}) = ${trieu(cost)} tiền dầu; lọc dầu sạch giúp bớt ${lo} tới ${hi}% (tài liệu SF300B) = ${trieu(saveLo)} tới ${trieu(saveHi)} một chuyến, ${a.tripsPerYear} chuyến một năm là ${trieu(saveLo * a.tripsPerYear)} tới ${trieu(saveHi * a.tripsPerYear)}; máy 9,X triệu hoàn vốn sau 2 tới 4 chuyến.`);
  }
...
export function waterBenefitLines() {
  ...
  out.push('CẤM suy ra "bớt chở nước", "nhẹ tàu", "đỡ tốn dầu" từ ví dụ này (sự thật nghề: tàu cố ý chở nước để đằm khi lấy đá). Không nêu tiền nước ở cảng đất liền vì chưa có giá công khai.');
  return out;
}
export function benefitLines(group) {
  ...
  return [
    'BÀI TOÁN LỢI ÍCH (BẮT BUỘC có 2 tới 3 câu trong bài, đặt ở nhịp lối thoát hoặc phần thưởng): chọn ĐÚNG 1 ví dụ dưới đây hợp cỡ tàu của tệp khách, CHÉP NGUYÊN các con số, KHÔNG tự nhân chia, KHÔNG thêm số ngoài ví dụ, KHÔNG làm tròn khác đi. Kèm 1 cụm nguồn ngắn trong ngoặc, ví dụ "(tính theo giá dầu Petrolimex kỳ 3/9/2026)".',
    ...ex,
  ];
}
```
...
ok('benefit lọc dầu có ví dụ tàu 15 m', fuelBenefitLines().some((l) => l.includes('tàu 15 m') && l.includes('27.740 đ') && l.includes('5 tới 10%')));
ok('benefit lọc dầu hoàn vốn', fuelBenefitLines().some((l) => l.includes('hoàn vốn sau 2 tới 4 chuyến')));
ok('benefit lọc nước 3 tấn cho tàu 15 m', waterBenefitLines().some((l) => l.includes('tàu 15 m') && l.includes('2.250 lít')));
...
```
Lưu ý: tàu 15 m 10 người 15 ngày × 15 lít = 2.250 lít (ví dụ Thanh nói "20 ngày 1 tấn" là mức chỉ uống nấu;
bảng dùng mức chở thật 15 lít nên ra 2,25 tấn cho 15 ngày; nếu Thanh muốn ví dụ 20 ngày thì đổi `days: 20`
ở case `tau-15m` và sửa test thành `3.000 lít`).

### Bước 5: doc + commit + deploy

- `docs/app-map/marketing.md` chèn sau `covers:`:
  `<!-- re-verified: 2026-09-11 toi - BAI TOAN LOI ICH trong bai ban (Thanh 11/9; so co nguon, Claude tim Google, xem docs/plans/plan-bai-toan-loi-ich-11-09.md): products.mjs (2 ban) BENEFIT_ASSUMPTIONS (gia dau DO 27.740 d/lit ky 3/9/2026, env DIESEL_PRICE_VND/DIESEL_PRICE_DATE; 15 lit nuoc/nguoi/ngay suy tu Tepbac 2018; tiet kiem 5-10% tai lieu SF300B; 6 chuyen/nam), BENEFIT_CASES 3 co tau, fuelBenefitLines/waterBenefitLines/benefitLines; social.mjs (2 ban) chen ...benefitLines(productGroup) sau ...audienceLines; test +7 ca. Loc nuoc KHONG neu tien nuoc dat lien (khong co nguon), khong noi bot cho nuoc/nhe tau. -->`
- `docs/app-map/README.md` chèn sau `covers:` một dòng tương tự ngắn cho bản apps.
- Commit: `feat(social): bai toan loi ich theo co tau trong bai ban (so co nguon, gia dau theo env) (Thanh 11/9); re-verify(docs/app-map/marketing.md, README.md)`.
...
1. `node --check` 4 file; `npm run test:price` ở gốc repo → 89/89 (82 sau plan trước + 7).
...
3. Sinh thử 2 bài như plan trước (script scratch) → bài lọc dầu có đoạn "2.000 lít dầu... 55,5 triệu đồng... 2,8 tới 5,5 triệu đồng một chuyến"; bài lọc nước có "2.250 lít... 113 can 20 lít... chạy khoảng 0,6 giờ mỗi ngày".
...
- [ ] BENEFIT_* + 3 hàm ở cả 2 products.mjs; `...benefitLines(productGroup)` ở cả 2 social.mjs.
- [ ] test:price 89/89; node --check sạch; gate OK; sinh thử 2 bài đúng số.
