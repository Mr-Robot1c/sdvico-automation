...
export async function buildRecruitmentPoster(input: PosterInput): Promise<Buffer | null> {
  try {
    ...
    const body = div({ display: 'flex', flexDirection: 'column', flexGrow: 1, justifyContent: 'center', padding: '40px 50px', gap: 26 }, [
      // Chỉ thêm khối lương nếu có (trích từ mô tả lúc soạn bài).
      ...(salary ? [div({ display: 'flex', alignItems: 'center', gap: 14, background: t.red, borderRadius: 14, padding: '16px 26px', alignSelf: 'flex-start', maxWidth: 980 }, [
        div({ display: 'flex', width: 44, height: 44, borderRadius: 10, background: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }, [imgEl(icon(MONEY), { width: 28, height: 28 })]),
        txt({ color: '#fff', fontSize: 25, fontWeight: 700 }, 'Mức lương:'),
        txt({ color: '#fff', fontSize: 34, fontWeight: 700 }, salary),
      ])] : []),
